...
def extractTagFromFile(jsonTag, jsonFile, pattern, file, count=False, occurrence=-1, defaultNotFound="N/A", t=str):
  ...
  try:
    ...
    if m := re.findall(pattern, content, re.M):
      if count:
        # Return the count
...
# ==============================================================================
  ...
  metrics_dict["total_time"] = "ERR" if failed else str(total)
...
#
  ...
  with open("metrics.html", "w") as metrics_html_file:
